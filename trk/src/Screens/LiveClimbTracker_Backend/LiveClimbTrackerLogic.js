    ...
    useEffect(() => {
        if (selectedGymId) {
            ...
            async function fetchClimbs() { 
                const climbsData = await getClimbsBySomeField('gym', selectedGymId);
    ...
    useEffect(() => {
        ...
        async function fetchTapsForClimbs() {
            ...
            for (const climb of climbs) {
                const tapsData = await getTapsByClimbAndDate(climb.value, selectedDate); //Selecing data with timestamp (Firebase query makes it faster)
